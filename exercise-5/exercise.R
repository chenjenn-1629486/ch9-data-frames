...
pop_female_name_2013 <- names_2013[names_2013$prop == max(names_2013$prop), "name"]
...
most_popular_in_year <- function(year) {
  ...
  most_pop <- year_names[year_names$prop == max(year_names$prop), 'name']
  most_pop
}

# What was the most popular female name in 1994?
pop_female_name_1994 <- most_popular_in_year(1994)
...
number_in_million <- function(name, year) {
  name_year <- names[names$year == year & names$name == name, 'prop']
  round(name_year*1000000, 1)
}
